...
// JWT payload에서 role 추출 (변조 여부와 무관하게 클라이언트 표시용)
...
export default function TransferPage() {
  ...
  // JWT의 role이 admin이면 강제 이체 패널 노출
  // → JWT 변조(role: "admin")로 일반 유저도 패널을 볼 수 있음 (BAC)
  ...
  // [VULN] BOLA: from_account_id 소유자 확인 없음 (서버 측)
  ...
  // [VULN] Broken Access Control: /admin/force 엔드포인트 (일반 유저도 접근 시도 가능)
  ...
  return (
    <Layout>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isAdmin ? '1fr 1fr' : '1fr', gap: 24, marginBottom: 24 }}>
        ...
        {/* 관리자 강제 이체 — JWT role=admin일 때만 렌더링 (BAC: JWT 변조로 노출 가능) */}
        {isAdmin && (
